 ...
 * Date:    2017.04.29
 *
 *   LogProv loader class. Load data, assign pipeline execution ID and tell PM about the loading specification.
 *
 * Syntax:
 *   [dstvar] = LOAD '[source URI]' USING com.logprov.pigUDF.ProvLoader('[dstvar]', '[column type]', '[inspected
 * columns]', '[pipeline monitor URL]', '[HDFS path]', '[remarks]');
 *
 *   [dstvar] is the name for output variable who receives data from loader; [column type] is a string indicating the
 * type of each column in data, where 'n' and 's' stands for numerical and textual data respectively, separated by ','
 * if many; [inspected columns] is a string indicating the indices of columns used as evidences to discover operation
 * malfunction, ',' separated if many; [HDFS path] is the absolute or relative path to where all intermediate data
 * locate as well as those used or generated by anomaly detection, and those produced by same scripts should adopt exact
 * path whilst different script should use different path; [remarks] is a string containing comments or remarks of the
 * pipeline.
 *   This loader should never be predefined with fixed initializer parameters, since some parameters may vary from time
 * to time.
 *
 * Example:
 *   ...
 *   raw = LOAD 'raw.dat' USING com.logprov.pigUDF.ProvLoader('raw', 'n,n,s,s' '0,2', 'http://localhost:58888',
 * 'InterMediateData', 'This is an example pipeline');
 *   ...
 *
 * TODO: Can this loader be pre-defined in Pig Latin Script?
